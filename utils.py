...
def split_dataset_by_study(data, feature_names, excluded_study_labels=(20,),excluded_participants=None):
  ...
  cols_to_remove = ["StartDate", "EndDate", "Platform", "IPAddress", "workerId", "Duration__in_seconds_",
                    ...
                    "long", "hard","crt_score3_original", "crt_score_5_original","filter_$"]
  ...
  for study_number in data.Study.unique():
    if not np.isnan(study_number) and study_number not in excluded_study_labels:
      ...
      if data_for_study.shape[0] > 0:
        X_for_study = data_for_study.drop(labels=[x for x in ["newcomb_combined", "Study","comprehension", "newcomb_confidence","believability_prediction", "believability_scenario", "believability_1", "believability_2"] if x in data_for_study.columns], axis=1)
